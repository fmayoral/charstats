...
CharStats.funciones.updateCharStats = function () {
  ...
  for (var i = 0; i < pj.habilidades.length; i++) {
    ...
    if (pj.habilidades[i].active || feat.pasive) {
      if (CharStats.funciones.aplicarEfecto(pj, feat)) {
        for (var j = 0; j < feat.efectos.length; j++) {
          CharStats.funciones.aplicarFeat(pj, feat.efectos[j]);
        };
      }
    }
  };
...
CharStats.funciones.aplicarEfecto = function (pj, habilidad){
  ...
  for (var j = 0; j < habilidad.condiciones.length; j++) {
    response = CharStats.funciones.checkCondicion(pj, habilidad.condiciones[j]) && response;
  };
  return response;
};
...
CharStats.funciones.modExperience = function (modValue){
  var pj = Personajes.findOne({'_id': Session.get('charName')});
